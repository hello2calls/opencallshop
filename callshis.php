...
if($_REQUEST['calendar']=="" || $_REQUEST['calendar1']==""){
	$query = "SELECT * FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." and call_start LIKE '%".date("Y-m-d")."%' ";
} else {
	$query = "SELECT * FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." AND (call_start between '".$d_from."' AND '".$d_to."' OR call_end between '".$d_from."' AND '".$d_to."')";
}
//echo $query;
...
if($_REQUEST['calendar']=="" || $_REQUEST['calendar1']==""){
	$query2 = "SELECT * FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." and call_start LIKE '%".date("Y-m-d")."%' ORDER BY call_end DESC limit ".$n->sql()."";

	$query3 = "SELECT sum( cost ) FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." and call_start LIKE '%".date("Y-m-d")."%'";

	$query4 = "SELECT sum( duration ) FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." and call_start LIKE '%".date("Y-m-d")."%'";

} else {
	$query2 = "SELECT * FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." AND (call_start between '".$d_from."' AND '".$d_to."' OR call_end between '".$d_from."' AND '".$d_to."') ORDER BY call_end DESC LIMIT ".$n->sql()."";

	$query3 = "SELECT sum( cost ) FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." AND (call_start between '".$d_from."' AND '".$d_to."' OR call_end between '".$d_from."' AND '".$d_to."')";

	$query4 = "SELECT sum( duration ) FROM callscallshop c WHERE paid = 1 AND id_client = ".$_SESSION["c_id"]." AND (call_start between '".$d_from."' AND '".$d_to."' OR call_end between '".$d_from."' AND '".$d_to."')";
}
//echo $query2;
...
if (mysql_num_rows($result2) > 0 )
{
   ...
   $row_counter = 0;	 
	...
	echo "<tr><td class=\"topt\" colspan=\"3\">".$n->show($op="{H}","","")."</td>
				...
				<td align=\"right\" class=\"topt\" colspan=\"2\">".TOTAL_COST."".$totalcost."</td></td><td class=\"topt\">&nbsp</td></tr>";
	echo "</table>";
} else {
